...
CLI **KHÔNG** tự giải mã bí mật trong Node.js. 
...
- Truyền nguyên cục mã hóa này vào Background Script của Extension qua API `chrome.runtime.sendMessage` hoặc qua thao tác chèn thẳng vào IndexedDB (`dbStorage.credentials`).
...
- Lợi ích: Node.js (Vùng đất nguy hiểm dễ bị hack script) không bao giờ chạm tay vào bản rõ của mật khẩu.
