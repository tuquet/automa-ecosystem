<template>
  <ui-card padding="p-0" :class="['workflow-settings', cardClass]">
    <div v-if="showTitle" class="flex items-center px-4 pt-4">
      <p class="flex-1 font-medium text-sm text-foreground">
        {{ t('common.settings') }}
      </p>
      <v-remixicon
        v-if="showClose"
    ...
    <ui-tab-panels
      v-model="activeTab"
      :class="['scroll settings-content overflow-auto p-4', contentClass]"
      :style="
        contentClass
          ? undefined
          : 'height: calc(100vh - 10rem); max-height: 600px'
      "
    >
...
watch(
  () => props.workflow?.settings,
  (newSettings) => {
    if (newSettings) {
      Object.assign(settings, cloneDeep(newSettings));
    }
  },
