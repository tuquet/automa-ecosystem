...
export class CliWorkerService {
  ...
  constructor(private configService: ConfigService) {
    this.workerUrl = this.configService.get<string>('CLI_WORKER_URL', 'http://127.0.0.1:3500/api/jobs/run');
  }
  ...
  async dispatchJob(payload: any): Promise<any> {
    ...
    try {
      ...
      throw new ServiceUnavailableException(`CLI Worker is unavailable: ${error.message}`);
    }
  }
}
