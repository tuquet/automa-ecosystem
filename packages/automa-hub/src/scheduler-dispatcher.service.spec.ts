...
describe('SchedulerDispatcherService', () => {
  ...
  describe('onModuleInit', () => {
    it('should register active schedules as cron jobs', async () => {
      const mockSchedules = [
        { id: 's1', cronExpr: '0 * * * *', status: 'active', fleetId: 'f1', workflowPath: '/path' },
        { id: 's2', cronExpr: '0 * * * *', status: 'paused', fleetId: 'f2', workflowPath: '/path2' }, // should be ignored
      ];
  ...
  describe('dispatchFleet', () => {
    it('should fetch members and dispatch to campaign service respecting concurrency', async () => {
      const mockSchedule = { id: 's1', fleetId: 'f1', workflowPath: '/my/workflow.json', concurrency: 2 };
      const mockMembers = [{ accountId: 'acc1' }, { accountId: 'acc2' }, { accountId: 'acc3' }];
      
      jest.spyOn(fleetService, 'getFleetMembers').mockResolvedValue(mockMembers as any);
      jest.spyOn(campaignService, 'runCampaign').mockResolvedValue(true as any);

      await service.dispatchFleet(mockSchedule as any);

      expect(fleetService.getFleetMembers).toHaveBeenCalledWith('f1');
