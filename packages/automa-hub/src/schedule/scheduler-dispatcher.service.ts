...
export class SchedulerDispatcherService implements OnModuleInit {
  ...
  addCronJobForSchedule(schedule: any) {
    ...
    try {
      const job = new CronJob(schedule.cronExpr, async () => {
        ...
        await this.dispatchCampaign(schedule);
      });
  ...
  async dispatchCampaign(schedule: any) {
    this.logger.log(`Dispatching campaign ${schedule.campaignId} for schedule ${schedule.id}`);
    
    try {
      const members = await this.campaignService.getCampaignAccounts(schedule.campaignId);
      
      if (!members || members.length === 0) {
        this.logger.warn(`No members found in campaign ${schedule.campaignId}`);
        return;
      }
      ...
      this.logger.error(`Error dispatching campaign ${schedule.campaignId}`, err);
    }
  }
}
