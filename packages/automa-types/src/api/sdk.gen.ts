...
/**
 * Ensure managed default Chromium browser profile
 *
 * Ensures standard managed Chromium browser profile is registered in SQLite and configured as default.
 */
...
/**
 * Terminate or stop an active browser session
 *
 * Gracefully closes the running browser process (or forcefully if force=true), sanitizes volatile caches, cleans up locks, and syncs profile data.
 */
export const stopBrowserSession = <ThrowOnError extends boolean = false>(options: Options<StopBrowserSessionData, ThrowOnError>): RequestResult<StopBrowserSessionResponses, StopBrowserSessionErrors, ThrowOnError> => (options.client ?? client).delete<StopBrowserSessionResponses, StopBrowserSessionErrors, ThrowOnError>({ url: '/api/v1/browsers/{id}/session', ...options });
...
/**
 * Export full Automa backup JSON
 *
 * Exports all workflows, storage variables, and storage tables from SQLite into a standardized Automa backup JSON format.
 */
export const exportStorageBackup = <ThrowOnError extends boolean = false>(options?: Options<ExportStorageBackupData, ThrowOnError>): RequestResult<ExportStorageBackupResponses, ExportStorageBackupErrors, ThrowOnError> => (options?.client ?? client).get<ExportStorageBackupResponses, ExportStorageBackupErrors, ThrowOnError>({ url: '/api/v1/storage/backup/export', ...options });

/**
 * Restore full Automa backup into SQLite
 *
 * Restores workflows, storage variables, and storage tables from an Automa backup JSON. Supports protected backups encrypted with AES-256-CBC and HMAC-SHA256.
 */
