...
- **No `webextension-polyfill`**: The extension uses native `chrome.*` API (MV3) and `browser.*` API (Firefox) via a minimal wrapper at `src/lib/browser-compat.js`.
  ...
  - **Daemon Polling & Reused Processes**: When connecting to an already running Daemon process (reusing the port), the `DaemonManager` must correctly track state via an `isExternalDaemon` flag. The `isRunning()` method must return true for external daemons to prevent `TaskRunner` polling loops from abruptly crashing with false disconnected errors.
  - **Chromium Version & Download Source**: CLI (`automa-cli`) uses `latest` Chromium build (`PuppeteerBrowser.CHROMIUM`), NOT Chrome for Testing. This Chromium executable is downloaded from Google Cloud Storage via `@puppeteer/browsers`, whereas the Automa Extension (`automa-ex`) is downloaded from GitHub Releases.
