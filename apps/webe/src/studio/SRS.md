...
> **Thư mục mã nguồn**: `apps/webe/src/studio/`  
...
Đặc biệt, tài liệu làm rõ cơ chế **Contract-First & Central Type Hub**: toàn bộ giao tiếp giữa Studio Standalone và Rust Daemon `apps/core` được định kiểu an toàn (100% Type-Safe) thông qua SDK Client tập trung **`@automa/types/api`**.
...
- **Build Target**: Được đóng gói độc lập thông qua `webpack.studio.config.js` xuất ra thư mục `apps/webe/dist/studio/`.
- **Daemon Web Server**: Được nhúng và phục vụ trực tiếp (natively hosted) bởi Daemon Rust `apps/core` tại endpoint tĩnh `http://127.0.0.1:8765/studio/` thông qua `tower_http::services::ServeDir`.
- **Client Consumers**:
  1. **Trình duyệt Web Tiêu chuẩn**: Chạy trên Google Chrome, Brave, MS Edge, Mozilla Firefox, Apple Safari.
  2. **VS Code Extension (`apps/vsce`)**: Kích hoạt nhanh thông qua lệnh `Open in Studio` hoặc task `[Preset] Live Studio`.
...
> [!IMPORTANT]
> **Quy tắc bất biến**: `apps/webe/src/studio` **TUYỆT ĐỐI KHÔNG** tự gọi `fetch()` với URL chuỗi cứng hay tự sinh API types riêng rẽ. Toàn bộ các tác vụ gọi Daemon (Workflow CRUD, Run Job, Live Lint, Storage Data) **BẮT BUỘC** thông qua SDK methods của `@automa/types/api`.
...
graph TD
    ...
    subgraph StudioFrontend [apps/webe (Studio Standalone)]
...
| :--- | :--- | :--- | :--- |
| `getWorkflow(options)` | [`StudioApp.vue`](StudioApp.vue), [`WorkflowLibraryModal.vue`](components/WorkflowLibraryModal.vue) | `GET /api/v1/storage/workflow` | Nạp nội dung kịch bản JSON từ thư mục `apps/vault` lên VueFlow canvas. |
...
| `listStorageFiles(options)` | [`services/storage.service.js`](services/storage.service.js) | `GET /api/v1/storage/files` | Truy xuất danh sách tệp kịch bản `.workflow.json` và `.campaigns.json` phục vụ cho modal Storage Explorer. |
| `getStorageTables(options)` / `addStorageTable` / `deleteStorageTable` | [`services/storage.service.js`](services/storage.service.js) | `/api/v1/storage/tables` | Quản lý bảng dữ liệu Global Storage, phân trang dữ liệu dòng (Rows). |
| `getStorageVariables(options)` / `addStorageVariable` / `deleteStorageVariable` | [`services/storage.service.js`](services/storage.service.js) | `/api/v1/storage/variables` | Quản lý danh sách biến toàn cục (Key-Value pairs). |
| `getStorageCredentials(options)` | [`services/storage.service.js`](services/storage.service.js) | `/api/v1/storage/credentials` | Lấy danh sách khóa bí mật mã hóa AES-256. |
| `client` (`createClient` instance) | [`services/storage.service.js`](services/storage.service.js) | Tùy biến endpoint qua `client.post/delete/patch` | Xử lý xả hàng đợi đồng bộ ngoại tuyến (`flushSyncQueue`) khi kết nối Daemon phục hồi. |
...
graph TB
    ...
    subgraph RustDaemon [apps/core Backend (Port: 8765)]
        StaticServe["/studio -> ServeDir(apps/webe/dist/studio)"]
        RestAPI["REST API (/api/v1/jobs, /storage, /lint, /browsers)"]
        SSEStream["SSE Event Bus (/api/v1/events, /jobs/worker/sse)"]
        SqliteDB[("SQLite Database + apps/vault Files")]
    end
...
- **FR-2.1 (Storage Explorer)**: Tích hợp modal duyệt tệp [`WorkflowLibraryModal.vue`](components/WorkflowLibraryModal.vue) kết nối với SDK `listStorageFiles` (`@automa/types/api`) để đọc danh sách workflow từ `apps/vault`.
...
- **FR-6.4 (Offline Sync Queue)**: Khi mất kết nối tới `apps/core`, mọi thao tác CRUD dữ liệu được lưu tạm vào hàng đợi `__automa_storage_sync_queue` trong `localStorage` và tự động flush đồng bộ lại ngay khi Daemon trực tuyến qua `client` SDK.
...
sequenceDiagram
    ...
    participant Explorer as WorkflowLibraryModal.vue
    participant SDK as @automa/types/api
    participant Core as apps/core (Port 8765)
    ...
    Core-->>SDK: 200 OK (File Persisted in apps/vault)
...
Sau khi khởi động `apps/core` (Port `8765`), truy cập đường dẫn:
```text
http://127.0.0.1:8765/studio/
```
Tại đây, trình duyệt sẽ tự động tải ứng dụng Studio Standalone, kết nối với kho kịch bản `apps/vault`, cơ sở dữ liệu `Global Storage` và cho phép thực thi quy trình trực tiếp thông qua `@automa/types/api`.
