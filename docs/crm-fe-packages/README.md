...
| ID (File JSON) | Tên Package | File JSON | Mô tả (Nhiệm vụ) | Đầu vào (Inputs) | Đầu ra (Outputs) | Đầu vào ngầm định (Implicit Inputs) | Dùng trong Workflow |
|---|---|---|---|---|---|---|---|
...
| `jzZa_ycRZs8XGsrz-HJwg` | `[form] Login` | [pkg-form-login.json](./pkg-form-login.json) | Sử dụng global data input form (Điền Username, Password, và bấm Đăng nhập) | `valid`, `invalid_credentials`, `invalid_regex` | `200`, `401`, `400` | `{{variables.$$loginUrl}}`, `{{variables.loginUsername}}`, `{{secrets@commonPassword}}` | [\[wf-login\] Login](../crm-fe-workflows/wf-login.json) |
...
| `pkg-nav-team-detail` | `[nav] Go to Team Detail` | [pkg-nav-team-detail.json](./pkg-nav-team-detail.json) | Chuyển hướng tới chi tiết Team. Yêu cầu truyền biến `{{variables.teamId}}` | `start` | `end` | `{{variables.teamId}}` | *(Chưa dùng)* |
| `pkg-nav-employee-detail` | `[nav] Go to Employee Detail` | [pkg-nav-employee-detail.json](./pkg-nav-employee-detail.json) | Chuyển hướng tới chi tiết Employee. Yêu cầu truyền biến `{{variables.employeeId}}` | `start` | `end` | `{{variables.employeeId}}` | *(Chưa dùng)* |
...
| `pkg-action-add-employee` | `[action] Add Employee` | [pkg-action-add-employee.json](./pkg-action-add-employee.json) | Thêm nhân viên mới vào hệ thống/team (Nhận biến `add_employee_ldap`) | `start` | `end` | `add_employee_ldap` | *(Chưa dùng)* |
...
| `pkg-action-select-codebook-category` | `[action] Select Category` | [pkg-action-select-codebook-category.json](./pkg-action-select-codebook-category.json) | Click vào một Category theo biến `{{variables.select_code_type}}` | `start` | `end` | `{{variables.select_code_type}}` | *(Chưa dùng)* |
...
| `pkg-action-add-codebook-property` | `[action] Add Property` | [pkg-action-add-codebook-property.json](./pkg-action-add-codebook-property.json) | Thêm giá trị danh mục mới dựa vào biến `add_property_code` và `add_property_name` | `start` | `end` | `add_property_code`, `add_property_name` | *(Chưa dùng)* |
